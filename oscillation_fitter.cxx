...
const std::string XCHECK_ROOT = ROOT_FILES_DIR + "xcheck.root";
const std::string EVENT_NORMAL_ROOT = ROOT_FILES_DIR + "event_normal_qiyubin.root";
...
std::vector<TH2D*> xcheck_event_hists;

std::vector<TH2D*> chi2_hists;
...
void initialize() {
    ...
    // Open EVENT_NORMAL_ROOT and load expected_event_hists
    ...
    for (const auto& flavor : flavor_names) {
        ...
        expected_event_hists.push_back(dynamic_cast<TH2D*>(hist->Clone())); // Clone the histogram
        TH2D* observed_hist = dynamic_cast<TH2D*>(hist->Clone(("observed_" + flavor).c_str()));
        observed_hist->Reset();
        observed_event_hists.push_back(observed_hist);
    }

    // event_normal_file->Close();
    // delete event_normal_file;

    // Open XCHECK_ROOT and load flux and cross-section histograms
    TFile* xcheck_file = TFile::Open(XCHECK_ROOT.c_str());
    if (!xcheck_file || xcheck_file->IsZombie()) {
        std::cerr << "Error: Could not open " << XCHECK_ROOT << std::endl;
        exit(1);
    }

    // Load flux histograms
    flux_histograms = {
        loadTH2D(XCHECK_ROOT, "flux_hist_numu"),
        loadTH2D(XCHECK_ROOT, "flux_hist_nue"),
        loadTH2D(XCHECK_ROOT, "flux_hist_numubar"),
        loadTH2D(XCHECK_ROOT, "flux_hist_nuebar")
    };

    // Load cross-section histograms for C12
    xsec_C12_histograms = {
        loadTH1D(XCHECK_ROOT, "xsec_hist_numu"),
        loadTH1D(XCHECK_ROOT, "xsec_hist_nue"),
        loadTH1D(XCHECK_ROOT, "xsec_hist_numubar"),
        loadTH1D(XCHECK_ROOT, "xsec_hist_nuebar")
    };

    // Load cross-section histograms for H1
    xsec_H1_histograms = {
        loadTH1D(XCHECK_ROOT, "xsec_hist_numu"),
        loadTH1D(XCHECK_ROOT, "xsec_hist_nue"),
        loadTH1D(XCHECK_ROOT, "xsec_hist_numubar"),
    ...
    // Load additional group of histograms into global xcheck_event_hists
    for (const auto& flavor : flavor_names) {
        std::string hist_name = "Prediction_hist_" + flavor;
        TH2D* hist = dynamic_cast<TH2D*>(xcheck_file->Get(hist_name.c_str()));
        if (!hist) {
            std::cerr << "Error: Could not load histogram " << hist_name << " from XCHECK_ROOT" << std::endl;
            exit(1);
        }
        xcheck_event_hists.push_back(dynamic_cast<TH2D*>(hist->Clone()));
    }

    for (int i = 0; i < 4; ++i) {
    chi2_hists.push_back(new TH2D(("chi2_hist_" + std::to_string(i)).c_str(), "Chi2 Histogram", 10, 0, 10, 12, 0, 12));
    }

    // xcheck_file->Close();
    // delete xcheck_file;
    
    // Log successful loading of histograms
    // std::cout << "Successfully loaded " << expected_event_hists.size() << " expected histograms." << std::endl;
    // std::cout << "Successfully loaded " << observed_event_hists.size() << " observed histograms." << std::endl;
    // std::cout << "Successfully loaded " << flux_histograms.size() << " flux histograms." << std::endl;
    // std::cout << "Successfully loaded " << xsec_C12_histograms.size() << " C12 cross-section histograms." << std::endl;
    // std::cout << "Successfully loaded " << xsec_H1_histograms.size() << " H1 cross-section histograms." << std::endl;
    // std::cout << "Successfully loaded " << xcheck_event_hists.size() << " additional histograms from XCHECK_ROOT." << std::endl;

    // Additional processing for xcheck_event_hists can be added here if needed
}
...
// Function to save xcheck (as expected) and observed histograms to a ROOT file
void save_rebinned_histograms(const std::string& output_file_name) {
    // Open a ROOT file for writing
    TFile output_file(output_file_name.c_str(), "RECREATE");
    if (!output_file.IsOpen()) {
    ...
    // Save xcheck histograms as expected histograms
    for (size_t i = 0; i < xcheck_event_hists.size(); ++i) {
        TH2D* hist = xcheck_event_hists[i];
        if (hist) {
            std::string hist_name = hist->GetName();
            if (unique_names.insert(hist_name).second) { // Check if the name is unique
                hist->Write();
                std::cout << "Saved xcheck histogram (as expected): " << hist_name << " to file." << std::endl;
            } else {
                std::cerr << "Warning: Duplicate histogram name " << hist_name
                          << " encountered. Skipping to avoid overwrite." << std::endl;
            }
        } else {
            std::cerr << "Warning: Null pointer encountered in xcheck_event_hists at index " << i << "." << std::endl;
        }
    }
    ...
    for (size_t i = 0; i < rebinned_observed_event_hists.size(); ++i) {
        ...
        if (hist) {
            ...
            if (unique_names.insert(hist_name).second) { // Check if the name is unique
                hist->Write();
                std::cout << "Saved observed histogram: " << hist_name << " to file." << std::endl;
            } else {
                std::cerr << "Warning: Duplicate histogram name " << hist_name
                          << " encountered. Skipping to avoid overwrite." << std::endl;
            }
...
// Function to save chi2 histograms to a ROOT file
void save_chi2_histograms(const std::string& output_file_name) {
    ...
    // Save chi2 histograms (up to the first 4)
    for (size_t i = 0; i < chi2_hists.size() && i < 4; ++i) {
        TH2D* hist = chi2_hists[i];
        if (hist) {
            std::string hist_name = hist->GetName();
            if (unique_names.insert(hist_name).second) { // Check if the name is unique
                hist->Write();
                std::cout << "Saved chi2 histogram: " << hist_name << " to file." << std::endl;
            } else {
                std::cerr << "Warning: Duplicate histogram name " << hist_name
                          << " encountered. Skipping to avoid overwrite." << std::endl;
            }
        } else {
            std::cerr << "Warning: Null pointer encountered in chi2_hists at index " << i << "." << std::endl;
        }
    }

    // Close the file
    output_file.Close();
    std::cout << "Chi2 histograms saved to " << output_file_name << "." << std::endl;
}
